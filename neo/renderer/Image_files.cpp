 ...
 * the typedefs FILE and int before you can include jpeglib.h.
...
*/
void R_WriteTGA(
    const char *filename, const byte *data, int width, int height, bool flipVertical, const char *basePath) {
    byte *buffer;
    int i;
...
static void LoadTGA(const char *name, byte **pic, int *width, int *height, ID_TIME_T *timestamp) {
    ...
    *pic = NULL;
    ...
    if (targa_header.colormap_type != 0) { common->Error("LoadTGA( %s ): colormaps not supported\n", name); }
    ...
    *pic = targa_rgba;
            ...
            for (column = 0; column < columns; column++) {
                ...
                switch (targa_header.pixel_size) {
                    ...
                    case 32: blue = *buf_p++;
                        ...
                        *pixbuf++ = alphabyte;
                        break;
                    default: common->Error("LoadTGA( %s ): illegal pixel_size '%d'\n", name, targa_header.pixel_size);
                        break;
                }
            }
        }
    ...
    if ((targa_header.attributes & (1 << 5))) { // image flp bit
        if (width != NULL && height != NULL) { R_VerticalFlip(*pic, *width, *height); }
    }
...
static void LoadDDS(const char *name, byte **pic, int *width, int *height, ID_TIME_T *timestamp) {
    ...
    *pic = NULL;
    ...
    if (fileSize < (sizeof(unsigned int) + sizeof(DDS_HEADER))) {
        common->Error("LoadDDS( %s ): incomplete file \n", name);
    }

    // Validate DDS file signature
    if (*(unsigned long *) (buf_p) != DDS_MAGIC) { common->Error("LoadDDS( %s ):  magic != 'DDS' \n", name); }

    dds_header = (DDS_HEADER *) (buf_p + sizeof(unsigned long));
    ...
    if (dds_header->size != sizeof(DDS_HEADER)
        ...
        common->Error("LoadDDS( %s ):  invalid header \n", name);
    }
...
void R_LoadImage(const char *cname, byte **pic, int *width, int *height, ID_TIME_T *timestamp, bool makePowerOf2) {
    ...
    if (ext == "tga") {
        LoadTGA(name.c_str(), pic, width, height, timestamp); // try tga
    }
    if (ext == "dds") {
        LoadDDS(name.c_str(), pic, width, height, timestamp); // try dds
    }
...
=======================
*/
bool R_LoadImagesCubeMap(
    const char *imgName, cubeFiles_t extensions, byte *pics[6], int *outSize, ID_TIME_T *timestamp) {
    int i, j;
    ...
    if (extensions == CF_CAMERA) { sides = cameraSides; }
    else { sides = axisSides; }

    // FIXME: precompressed cube map files
    if (pics) { memset(pics, 0, 6 * sizeof(pics[0])); }
    ...
    for (i = 0; i < 6; i++) {
        ...
        else { R_LoadImageProgram(fullName, &pics[i], &width, &height, &thisTime); }
        ...
        if (timestamp) { if (thisTime > *timestamp) { *timestamp = thisTime; } }
    ...
    if (i != 6) {
        ...
        if (pics) { for (j = 0; j < i; j++) { R_StaticFree(pics[j]); } }
...
=======================
*/
bool R_LoadDDSCubeMap(const char *imgName, cubeFiles_t extensions, byte *pics[6], int *outSize, ID_TIME_T *timestamp) {
    int fileSize;
    byte *buffer;
    DDS_HEADER *dds_header;
    ...
    // load the file
    //
    fileSize = fileSystem->ReadFile(imgName, (void **) &buffer, timestamp);
    if (!buffer) { return false; }

    // Validate DDS file in memory.
    if (fileSize < (sizeof(unsigned int) + sizeof(DDS_HEADER))) {
        common->Error("LoadDDSCubeMap( %s ): incomplete file \n", imgName);
    }

    // Validate DDS file signature
    if (*(unsigned long *) (buffer) != DDS_MAGIC) { common->Error("LoadDDS( %s ):  magic != 'DDS' \n", imgName); }

    dds_header = (DDS_HEADER *) (buffer + sizeof(unsigned long));
...
bool R_LoadCubeImages(const char *imgName, cubeFiles_t extensions, byte *pics[6], int *outSize, ID_TIME_T *timestamp) {
    ...
    idStr ext;
    name.ExtractFileExtension(ext);
    if (ext == "dds" || extensions == CF_DDS) {
        return R_LoadDDSCubeMap(imgName, extensions, pics, outSize, timestamp);
    }
    else { return R_LoadImagesCubeMap(imgName, extensions, pics, outSize, timestamp); }
}
