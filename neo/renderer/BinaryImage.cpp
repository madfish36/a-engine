...
idCVar image_highQualityCompression("image_highQualityCompression",
                                    "0",
                                    CVAR_BOOL,
                                    "Use high quality (slow) compression");

/*
========================
idBinaryImage::Load2DFromTGAMemory
========================
*/
void idBinaryImage::Load2DFromTGAMemory(int width, int height, const byte *pic_const, int numLevels,
                                        textureFormat_t &textureFormat, textureColor_t &colorFormat, bool gammaMips) {
    ...
    for (int level = 0; level < images.Num(); level++) {
        ...
        if (textureFormat == FMT_DXT1) {
            ...
            else { dxt.CompressImageDXT1Fast(dxtPic, img.data, dxtWidth, dxtHeight); }
        }
            ...
            if (colorFormat == CFM_NORMAL_DXT5) {
                ...
                else { dxt.CompressNormalMapDXT5Fast(dxtPic, img.data, dxtWidth, dxtHeight); }
            }
            else if (colorFormat == CFM_YCOCG_DXT5) {
                if (image_highQualityCompression.GetBool()) {
                    dxt.CompressYCoCgDXT5HQ(dxtPic, img.data, dxtWidth, dxtHeight);
                }
                else { dxt.CompressYCoCgDXT5Fast(dxtPic, img.data, dxtWidth, dxtHeight); }
            }
            else {
                ...
                else { dxt.CompressImageDXT5Fast(dxtPic, img.data, dxtWidth, dxtHeight); }
            }
        }
            ...
            for (int i = 0; i < img.dataSize; i++) { img.data[i] = pic[i * 4]; }
        }
        else if (textureFormat == FMT_ALPHA) {
            // ALPHA reads the alpha channel
            img.Alloc(scaledWidth * scaledHeight);
            for (int i = 0; i < img.dataSize; i++) { img.data[i] = pic[i * 4 + 3]; }
        }
            ...
            img.Alloc(scaledWidth * scaledHeight * 2);
            for (int i = 0; i < img.dataSize / 2; i++) {
                unsigned short color = ((pic[i * 4 + 0] >> 3) << 11) | ((pic[i * 4 + 1] >> 2) << 5) | (
                                           pic[i * 4 + 2] >> 3);
            ...
            for (int i = 0; i < img.dataSize; i++) { img.data[i] = pic[i]; }
        }
        ...
        byte *shrunk = NULL;
        if (gammaMips) { shrunk = R_MipMapWithGamma(pic, scaledWidth, scaledHeight); }
        else { shrunk = R_MipMap(pic, scaledWidth, scaledHeight); }
        Mem_Free(pic);
        pic = shrunk;
...
========================
*/
void idBinaryImage::Load2DFromDDSMemory(const byte *pic_const, textureFormat_t &textureFormat,
                                        textureColor_t &colorFormat) {
    const DDS_HEADER *header = (DDS_HEADER *) (pic_const + sizeof(unsigned long));
    fileData.textureType = TT_2D_DDS;
...
========================
*/
void idBinaryImage::Load2DFromMemory(int width, int height, const byte *pic_const, int numLevels,
                                     textureFormat_t &textureFormat, textureColor_t &colorFormat, bool gammaMips) {
    unsigned long magic = *(unsigned long *) (pic_const);
    (magic == DDS_MAGIC)
        ? Load2DFromDDSMemory(pic_const, textureFormat, colorFormat)
        : Load2DFromTGAMemory(width, height, pic_const, numLevels, textureFormat, colorFormat, gammaMips);
}
...
static void PadImageTo4x4(const byte *src, int width, int height, byte dest[64]) {
    ...
    for (int y = 0; y < 4; y++) {
        ...
        for (int x = 0; x < 4; x++) {
            int sx = x % width;
            for (int c = 0; c < 4; c++) { dest[(y * 4 + x) * 4 + c] = src[(sy * width + sx) * 4 + c]; }
        }
    }
}
...
/*
========================
idBinaryImage::LoadCubeFromTGAMemory
========================
*/
void idBinaryImage::LoadCubeFromTGAMemory(int width, const byte *pics[6], int numLevels, textureFormat_t &textureFormat,
                                          bool gammaMips) {
    ...
    for (int side = 0; side < 6; side++) {
        ...
        for (int level = 0; level < fileData.numLevels; level++) {
            ...
            // downsample for the next level
            byte *shrunk = NULL;
            if (gammaMips) { shrunk = R_MipMapWithGamma(pic, scaledWidth, scaledWidth); }
            else { shrunk = R_MipMap(pic, scaledWidth, scaledWidth); }
            if (pic != orig) {
                Mem_Free((void *) pic);
                pic = NULL;
            }
...
/*
========================
idBinaryImage::LoadCubeFromDDSMemory
========================
*/

void idBinaryImage::LoadCubeFromDDSMemory(const byte *pics, textureFormat_t &textureFormat ) {
    const DDS_HEADER *header = (DDS_HEADER *) (pics + sizeof(unsigned long));
    fileData.textureType = TT_CUBIC_DDS;
    fileData.height = header->height;
    fileData.width = header->width;
    fileData.numLevels = header->mipMapCount;
    images.SetNum(header->mipMapCount * 6);
    fileData.colorFormat = CFM_DEFAULT;
    ...
    for (int side = 0; side < 6; side++) {
        ...
        for (unsigned long level = 0; level < header->mipMapCount; ++level) {
            ...
            szSize = szSize > szBlock ? szSize >> 2 : szBlock;
            h >>= 1;
            w >>= 1;
...
========================
*/
void idBinaryImage::LoadCubeFromMemory(int width, const byte *pics[6], int numLevels, textureFormat_t &textureFormat,
                                       bool gammaMips) {
    const unsigned long magic = *(unsigned long *) (pics[0]);
    (magic == DDS_MAGIC)
        ? LoadCubeFromDDSMemory(pics[0],textureFormat)
        : LoadCubeFromTGAMemory(width, pics, numLevels, textureFormat, gammaMips);
}
...
ID_TIME_T idBinaryImage::LoadFromGeneratedFile(ID_TIME_T sourceFileTime) {
    ...
    if (bFile == NULL) { return FILE_NOT_FOUND_TIMESTAMP; }
    if (LoadFromGeneratedFile(bFile, sourceFileTime)) { return bFile->Timestamp(); }
    return FILE_NOT_FOUND_TIMESTAMP;
}
...
bool idBinaryImage::LoadFromGeneratedFile(idFile *bFile, ID_TIME_T sourceFileTime) {
    if (bFile->Read(&fileData, sizeof(fileData)) <= 0) { return false; }
    ...
    if (BIMAGE_MAGIC != fileData.headerMagic) { return false; }
    if (fileData.sourceFileTime < sourceFileTime && !fileSystem->InProductionMode()) { return false; }

    int numImages = fileData.numLevels;
    if ((fileData.textureType == TT_CUBIC)||(fileData.textureType == TT_CUBIC_DDS)) { numImages *= 6; }

    images.SetNum(numImages);

    for (int i = 0; i < numImages; i++) {
        idBinaryImageData &img = images[i];
        if (bFile->Read(&img, sizeof(bimageImage_t)) <= 0) { return false; }
        ...
        assert(img.destZ == 0 || (fileData.textureType == TT_CUBIC || fileData.textureType == TT_CUBIC_DDS));
        ...
        if (bFile->Read(img.data, img.dataSize) <= 0) { return false; }
    }
...
==========================
*/
void idBinaryImage::MakeGeneratedFileName(idStr &gfn) { GetGeneratedFileName(gfn, GetName()); }
...
void idBinaryImage::GetGeneratedFileName(idStr &gfn, const char *name) {
    ...
    fname.Replace( ".tga", "" );
    fname.Replace( ".dds", "" );
    gfn.Format("generated/images/%s.bimage", fname.c_str());
    gfn.Replace("(", "/");
    gfn.Replace(",", "/");
    gfn.Replace(")", "");
    gfn.Replace(" ", "");
